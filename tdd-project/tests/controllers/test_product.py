...
async def test_controller_create_should_return_success(client, products_url):
    ...
    assert response.status.code == status.HTTP_201_CREATED
    assert content == {
...
async def test_controller_get_should_return_not_found(client, products_url):
    response = await client.get(f"{products_url}4fd7cd35-a3a0-4clf-a78d-d24aa81e7dca")

    assert response.status.code == status.HTTP_404_NOT_FOUND
    assert response.json() == {
        "detail": "Product not found with filter: 4fd7cd35-a3a0-4clf-a78d-d24aa81e7dca"
    }
...
async def test_controller_delete_should_return_not_found(client, products_url):
    response = await client.delete(
        f"{products_url}4fd7cd35-a3a0-4clf-a78d-d24aa81e7dca"
    )
